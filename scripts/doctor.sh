...
pass() { [[ "$JSON_OUTPUT" == false ]] && echo -e "${GREEN}ok${NC}   $1" || true; }
warn() { [[ "$JSON_OUTPUT" == false ]] && echo -e "${YELLOW}warn${NC} $1" || true; EXIT_CODE=1; }
fail() { [[ "$JSON_OUTPUT" == false ]] && echo -e "${RED}FAIL${NC} $1" || true; EXIT_CODE=2; }
info() { [[ "$JSON_OUTPUT" == false ]] && echo -e "${CYAN}info${NC} $1" || true; }
...
if [[ -d .sqry/graph ]]; then
  ...
  if [[ -f .sqry/graph/manifest.json ]]; then
    ...
    snapshot_magic=$(head -c 14 .sqry/graph/snapshot.sqry 2>/dev/null || true)
    ...
    if [[ -n "$snapshot_magic" ]]; then
      info "snapshot.sqry magic: $snapshot_magic (v$EXPECTED_VERSION writes SQRY_GRAPH_V17; older snapshots upconvert on load)"
      record "snapshot_magic" "info" "$snapshot_magic"
    fi
    ...
    if [[ "$VERBOSE" == true && "$JSON_OUTPUT" == false ]]; then
      echo "  manifest:"
      head -c 500 .sqry/graph/manifest.json
      echo ""
    fi
  else
  ...
  if command -v sqry >/dev/null 2>&1; then
    ...
    if echo "$status_json" | jq -e '.error' >/dev/null 2>&1; then
      ...
      files=$(echo "$status_json" | jq -r '.file_count // 0')
      symbols=$(echo "$status_json" | jq -r '.symbol_count // 0')
      stale=$(echo "$status_json" | jq -r '.stale // false')
      pass "Index reachable: $files files, $symbols symbols (stale=$stale)"
      record "index_status" "ok" "$files files, $symbols symbols, stale=$stale"
      if [[ "$stale" == "true" ]]; then
        warn "sqry reports the index as stale; run 'sqry update .' (or 'sqry index --force .')"
        record "index_stale" "warn" "stale=true"
      fi
...
MCP_FOUND=false
for cfg in .claude.json ~/.claude.json ~/.grok/config.toml ~/.codex/config.toml ~/.gemini/settings.json ~/.vibe/config.toml ~/.config/opencode/opencode.json opencode.json; do
  if [[ -f "$cfg" ]] && grep -q 'sqry-mcp' "$cfg" 2>/dev/null; then
    pass "sqry-mcp entry found in $cfg"
    MCP_FOUND=true
    record "mcp_config:$cfg" "ok" "sqry-mcp present"
  fi
done

# sqry mcp status --json reports the tools sqry itself configures (claude, codex, gemini).
if command -v sqry >/dev/null 2>&1; then
  mcp_status_json=$(sqry mcp status --json 2>/dev/null || echo '{}')
  configured_tools=$(echo "$mcp_status_json" | jq -r '
    [ .tools // {} | to_entries[]
      | select((.value.configured == true)
               or (.value.global.configured == true)
               or ((.value.projects // {}) | to_entries | map(.value.configured == true) | any))
      | .key ] | join(",")' 2>/dev/null || true)
  if [[ -n "$configured_tools" ]]; then
    pass "sqry mcp status reports configured hosts: $configured_tools"
    MCP_FOUND=true
    record "mcp_status_cli" "ok" "$configured_tools"
  fi
fi

if [[ "$MCP_FOUND" == false ]]; then
  warn "No sqry MCP config found in common locations. Enable the sqry plugin or run 'sqry mcp setup --tool <claude|codex|gemini|all>'"
...
if [[ -f .claude-plugin/plugin.json ]]; then
  if [[ -d skills/sqry-semantic-search ]]; then
    ...
    warn "skills/ subdir missing; plugin checkout may be incomplete"
    record "skills_layout" "warn" "missing"
  fi
else
  record "skills_layout" "skip" "not in plugin dir"
fi

# 6. Quick functional test (narrow query)
section "Quick Functional Test"
if command -v sqry >/dev/null 2>&1 && [[ -d .sqry/graph ]]; then
  if sqry query --json --limit 5 'kind:function' 2>/dev/null | jq -e '.results | length >= 0' >/dev/null 2>&1; then
...
if [[ "$JSON_OUTPUT" == true ]]; then
  ...
  echo "  sqry doctor channels        # if a stable and a dev sqry channel are both installed"
fi
