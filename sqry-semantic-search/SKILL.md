...
The sqry MCP server provides 34 tools. Tool names use the `mcp__sqry__` prefix in Claude Code, or equivalent for your agent.
...
| `expand_cache_status` | Macro expansion cache status (Rust) | `path` |
...
- The specific plugin was enabled with `--enable-plugin <id>`
...
MCP responses are redacted by default (`SQRY_REDACTION_PRESET=minimal`). To get unredacted output, set `SQRY_REDACTION_PRESET=none`. Available presets: `none`, `minimal`, `standard`, `strict`.
...
Current graph snapshot format is V7. Users must rebuild the index (`sqry index --force .`) when upgrading across major sqry versions.
...
**Domain-Specific**: SQL, Terraform, Puppet, Pulumi, Shell, HTML, CSS, Vue, Svelte, JSON

**Enterprise**: Salesforce Apex, SAP ABAP, ServiceNow Xanadu, ServiceNow XML, Oracle PL/SQL

Note: JSON and ServiceNow XML are classified as `HighWallClock` and excluded from the default index. See "Plugin Cost Tiering" above.
